...
class Chatbot {
    ...
    removeAttribution(text) {
        return text
            ...
            .replace(/[Gg]oogle/gi, '')
            .replace(/[Cc]ohere/gi, '')
            .replace(/\n\s*\*[^*]*\*/g, '') // Remove any line with asterisks
            .replace(/\s*\*[^*]*\*/g, '') // Remove any asterisk-wrapped text
            .trim();
    }

    addMessage(message) {
        // Remove any attribution text from bot messages
        if (message.type === 'bot') {
            message.content = this.removeAttribution(message.content);
        }
        ...
        if (message.type === 'bot') {
            ...
            // Check for attribution after typewriter effect
            setTimeout(() => {
                const finalText = textElement.textContent;
                if (finalText.includes('Powered by') || finalText.includes('google') || finalText.includes('cohere')) {
                    // Remove attribution from the displayed text
                    textElement.textContent = this.removeAttribution(finalText);
                }
            }, 100);
            
            // Also check periodically for any new attribution text
            const checkForAttribution = () => {
                const currentText = textElement.textContent;
                if (currentText.includes('*Powered by') || currentText.includes('*Generated by')) {
                    textElement.textContent = this.removeAttribution(currentText);
                }
            };
            
            // Check every 50ms for 2 seconds
            const attributionInterval = setInterval(checkForAttribution, 50);
            setTimeout(() => clearInterval(attributionInterval), 2000);
        } else {
